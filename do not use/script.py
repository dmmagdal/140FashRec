...
def ParseData(data_file):
  ...
  for item in images:
    ...
    if id_ in annotations:
        lid = id_
        id_ = "id_{}".format(id_)
        labels = "labelId{}".format(annotations[lid])
    key_url_list.append((id_, url, labels))
...
def DownloadImage(key_url):
  ...
  filename2 = os.path.join(out_dir, '%s.txt' % key)
  ...
  if os.path.exists(filename2):
    print('Text file %s already exists. Skipping download.' % filename2)
    return
  ...
  textwriting = open(filename2, 'w')
  for item in label:
    textwriting.write("%s" % item)
  textwriting.close()
