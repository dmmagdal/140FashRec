# tac.py
# tests the accuracy of the inception net trained graph 
# looks at top 22 of labels from running the label_image.py script
...
def matchLabels(labelist, resultsLabels, headerForFile, accuracyfile):
	...
	count = 0
	for i in range(1, 23):
		# label number from top 22 results.txt list
		stringOfLine = resultsLabels[startIndex+i].split(" ")
		#print(stringOfLine)
		#print(labelist)
		#break
		# if the label number exists in the valid label list
		if stringOfLine[0] in labelist:
			# increment the counter
			count += 1
	# write the accuracy for the file in the accuracy file
	accuracyfile.write(headerForFile + " accuracy = " + str(count/len(labelist) * 100) + "%\n")
...
def main():
	...
	for validfile in fileList:
		...
		headerForFile = headerStrings[headerStrings.index(str("**" + str(sys.argv[1]) + "/" + str(validfile).split(".jpeg.txt")[0] + ".jpg**"))]
